...
main:
	...
	;Puertos
	SER R16
	;Puerto A para adc, solo se ocupa PA0
	OUT DDRD,R16
	SBI DDRC,5
	SBI DDRC,3
	SBI DDRB,3
	...
	LDI R16,62	;SCL de 10kHz, prescaler 1 
	OUT TWBR,R16
	...
	fin:
		SBI ADCSRA,ADSC
		pollingADC:
			SBIS ADCSRA,ADIF	;Esperar a que se termine de convertir la lecutra
			RJMP pollingADC
		SBI ADCSRA,ADIF
		//IN R16,ADCL
		IN R24,ADCH	;Decidimos despreciar los dos primeros bits y ocupar la parte alta, es una relación lineal al fin de cuentas
		OUT PORTD,R24
		CPI R25,0b00000001	;Checar si se recibe este byte del master
		BREQ errorSTOP	;Detener PWM si se recibió
		OUT OCR0,R25	;Duty Cycle, cambiar duty cycle de acuerdo con el byte recibido del master
		RJMP fin
		errorSTOP:
			SBI PORTC,6	;indicar que se detuvo
			CLR R16
			OUT TCCR0,R16	;Apagar timer de pwm
		RJMP fin
...
stop:
	...
	SBI PORTC,6
	CLR R16
	OUT TCCR0,R16	;apagar timer pwm
